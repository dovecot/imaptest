...
static void pop3_command_send(struct pop3_client *client, const char *cmdline, pop3_command_callback_t *callback) {
...
static void pop3_command_finish(struct pop3_client *client, struct pop3_command *cmd) {
...
static int pop3_client_input_line(struct pop3_client *client, const char *line) {
  ...
  if (array_count(&client->commands) == 0) {
    pop3_client_input_error(client, "Received input while no commands were running");
    return -1;
  }
...
static void pop3_client_input(struct client *_client) {
  ...
  while ((line = i_stream_next_line(_client->input)) != NULL) {
    ...
    T_BEGIN { ret = pop3_client_input_line(client, line); }
    T_END;
    if (ret < 0) {
...
static int pop3_client_output(struct client *_client ATTR_UNUSED) { return 0; }

static void pop3_client_connected(struct client *_client ATTR_UNUSED) {}

static void pop3_client_authenticated(struct pop3_client *client, struct pop3_command *cmd) {
...
static int pass_callback(struct pop3_client *client, struct pop3_command *cmd, const char *line) {
  if (line[0] != '+') {
...
static int user_callback(struct pop3_client *client, struct pop3_command *cmd ATTR_UNUSED, const char *line) {
  struct client *_client = &client->client;
  const char *str;
...
static void pop3_client_login(struct pop3_client *client) {
  ...
  client->client.state = do_rand(STATE_AUTHENTICATE) ? STATE_AUTHENTICATE : STATE_LOGIN;
...
static int uidl_callback(struct pop3_client *client, struct pop3_command *cmd ATTR_UNUSED, const char *line) {
  const char *uidl;
  ...
  if (!array_is_created(&client->uidls)) {
    ...
    if (client->uidls_matched || client->client.user_client == NULL ||
      ...
      const char *const *old_uidlp = array_idx(&client->client.user_client->pop3_uidls, idx);
...
static int dele_callback(struct pop3_client *client, struct pop3_command *cmd ATTR_UNUSED, const char *line) {
  if (line[0] != '+') {
...
static int retr_callback(struct pop3_client *client, struct pop3_command *cmd ATTR_UNUSED, const char *line) {
  if (!client->retr_reading) {
    ...
    if (!client->pop3_keep_mails) {
      ...
      pop3_command_send(client, t_strdup_printf("DELE %u", client->prev_seq), dele_callback);
    }
...
static int quit_callback(struct pop3_client *client, struct pop3_command *cmd ATTR_UNUSED, const char *line) {
  if (line[0] != '+') {
  ...
  switch (client->client.login_state) {
    ...
    case LSTATE_SELECTED:
      if (!array_is_created(&client->uidls)) {
        ...
        pop3_command_send(client, t_strdup_printf("RETR %u", client->prev_seq + 1), retr_callback);
      } else {
...
static void pop3_client_free(struct client *_client) {
  ...
  if (uc != NULL && client->uidls_matched) {
    ...
    array_delete(&client->uidls, client->prev_seq, array_count(&client->uidls) - client->prev_seq);
...
static const struct client_vfuncs pop3_client_vfuncs = {.input = pop3_client_input,
...
struct pop3_client *pop3_client_new(unsigned int idx, struct user *user, struct user_client *uc) {
