...
const char *mail_flag_names[] = {"\\Answered", "\\Flagged", "\\Deleted", "\\Seen", "\\Draft", "\\Recent"};

static int metadata_static_cmp(const uint32_t *uidp, struct message_metadata_static *const *ms) {
  return *uidp < (*ms)->uid ? -1 : (*uidp > (*ms)->uid ? 1 : 0);
}

struct message_metadata_static *message_metadata_static_lookup_seq(struct mailbox_view *view, uint32_t seq) {
...
void message_metadata_static_unref(struct mailbox_storage *storage, struct message_metadata_static **_ms) {
  ...
  *_ms = NULL;
  ...
  if (!ms->expunged) {
    ...
    if (storage->static_metadata_ref0_count++ == 0) {
      storage->static_metadata_ref0_next_scan = ioloop_time + MESSAGE_STATIC_REF0_KEEP_SECS;
    }
    return;
  }

  if (!array_bsearch_insert_pos(&storage->static_metadata, &ms->uid, metadata_static_cmp, &idx))
    i_unreached();
  else
...
struct message_metadata_static *message_metadata_static_get(struct mailbox_storage *storage, uint32_t uid) {
  struct message_metadata_static **base, *ms;
  const struct seq_range *range;
  unsigned int count, idx;
  uint32_t first_uid;

  if (storage->static_metadata_ref0_count > 0 && ioloop_time >= storage->static_metadata_ref0_next_scan)
    message_metadata_static_free_old(storage);

  base = array_get_modifiable(&storage->static_metadata, &count);
  if (bsearch_insert_pos(&uid, base, count, sizeof(*base), metadata_static_cmp, &idx)) {
    ms = base[idx];
    if (ms->refcount++ == 0) {
  ...
  if (count > 32 && first_uid > 2 && range[0].seq2 < first_uid - 1) {
    seq_range_array_add_range(&storage->expunged_uids, 1, first_uid - 1);
  }
...
void message_metadata_static_assign_owner(struct mailbox_storage *storage, struct message_metadata_static *ms) {
...
static void mailbox_keywords_ref(struct mailbox_view *view, const uint8_t *bitmask) {
...
static void mailbox_keywords_drop(struct mailbox_view *view, const uint8_t *bitmask) {
  struct mailbox_keyword *keywords;
  unsigned int i, count;
...
void mailbox_view_expunge(struct mailbox_view *view, unsigned int seq) {
  ...
  if (metadata->ms != NULL) {
    seq_range_array_add(&view->storage->expunged_uids, metadata->ms->uid);
...
bool mailbox_view_keyword_find(struct mailbox_view *view, const char *name, unsigned int *idx_r) {
...
struct mailbox_keyword *mailbox_view_keyword_get(struct mailbox_view *view, unsigned int idx) {
  i_assert(idx < array_count(&view->keywords));
  return array_idx_modifiable(&view->keywords, idx);
}

struct mailbox_keyword *mailbox_view_keyword_get_by_name(struct mailbox_view *view, const char *name) {
  unsigned int idx;
...
static struct mailbox_keyword_name *mailbox_keyword_name_get(struct mailbox_storage *storage, const char *name) {
...
void mailbox_keywords_clear(struct mailbox_view *view, struct message_metadata_dynamic *metadata) {
  if (view->keyword_bitmask_alloc_size == 0)
    return;

  if (metadata->keyword_bitmask == NULL) {
    metadata->keyword_bitmask = i_malloc(view->keyword_bitmask_alloc_size);
  } else {
    mailbox_keywords_drop(view, metadata->keyword_bitmask);
  }
  memset(metadata->keyword_bitmask, 0, view->keyword_bitmask_alloc_size);
}

void mailbox_view_keywords_realloc(struct mailbox_view *view, unsigned int new_alloc_size) {
  struct message_metadata_dynamic *metadata;
  ...
  for (i = 0; i < count; i++) {
    metadata[i].keyword_bitmask = i_realloc(metadata[i].keyword_bitmask, old_alloc_size, new_alloc_size);
  }
}
...
void mailbox_view_keywords_write(struct mailbox_view *view, const uint8_t *bitmask, string_t *str) {
  const struct mailbox_keyword *keywords;
  unsigned int i, count;
...
const char *mailbox_view_keywords_to_str(struct mailbox_view *view, const uint8_t *bitmask) {
  string_t *str;
...
const char *mailbox_view_get_random_flags(struct mailbox_view *view, unsigned int client_idx) {
  struct mailbox_storage *storage = view->storage;
  static const char *keywords[] = {"$Label1", "$Label2", "$Label3", "$Label4", "$Label5"};
  struct mailbox_keyword *kw;
  unsigned int i, idx;
  string_t *str;

  if (!storage->flag_owner_clients_assigned && storage->assign_flag_owners) {
    i = 0;
    for (; i < N_ELEMENTS(storage->flags_owner_client_idx1); i++) {
      storage->flags_owner_client_idx1[i] = clients_get_random_idx() + 1;
    }
  ...
  for (i = 0; i < N_ELEMENTS(storage->flags_owner_client_idx1); i++) {
    ...
    if (storage->assign_flag_owners && storage->flags_owner_client_idx1[i] != client_idx + 1) {
  ...
  if (!view->keywords_can_create_more && array_count(&view->keywords) == 0) {
  ...
  for (i = 0; i < N_ELEMENTS(keywords); i++) {
    ...
    if (storage->assign_flag_owners && kw != NULL && kw->name->owner_client_idx1 != client_idx + 1) {
...
static void mailbox_metadata_free(struct mailbox_storage *storage, ARRAY_TYPE(message_metadata_dynamic) * messages) {
  struct message_metadata_dynamic *metadata;
  unsigned int i, count;
...
static struct mailbox_offline_cache *mailbox_offline_cache_alloc(struct mailbox_storage *storage) {
...
struct mailbox_storage *mailbox_storage_lookup(struct mailbox_source *source, const char *username,
                                               const char *mailbox) {
...
struct mailbox_storage *mailbox_storage_get(struct mailbox_source *source, const char *username, const char *mailbox) {
  struct mailbox_storage *storage;
  const char *guid;
...
void mailbox_storage_reset(struct mailbox_storage *storage) {
  ...
  memset(storage->flags_owner_client_idx1, 0, sizeof(storage->flags_owner_client_idx1));
...
bool mailbox_view_save_offline_cache(struct mailbox_view *view) {
  ...
  cache = view->storage->cache = mailbox_offline_cache_alloc(view->storage);
  ...
  for (i = 0; i < count; i++) {
    ...
    if (metadata[i].keyword_bitmask != NULL) {
      new_metadata.keyword_bitmask = i_malloc(keyword_bytecount);
      memcpy(new_metadata.keyword_bitmask, metadata[i].keyword_bitmask, keyword_bytecount);
    }
...
void mailbox_view_restore_offline_cache(struct mailbox_view *view, struct mailbox_offline_cache *cache) {
  ...
  for (i = 0; i < count; i++) {
    new_metadata = metadata[i];
    if (metadata[i].keyword_bitmask != NULL) {
      new_metadata.keyword_bitmask = i_malloc(view->keyword_bitmask_alloc_size);
      memcpy(new_metadata.keyword_bitmask, metadata[i].keyword_bitmask, view->keyword_bitmask_alloc_size);
  ...
  for (i = 0; i < count; i++) {
    new_kw = mailbox_view_keyword_get_by_name(view, keywords[i].name->name);
...
static bool mailbox_global_parse_envelope(struct mailbox_source *source, struct message_global *msg) {
  ...
  if (!imap_envelope_parse(msg->envelope, pool_datastack_create(), &env, &error)) {
  ...
  if (subject != NULL) {
    tmp = t_str_new(128);
    message_header_decode_utf8((const unsigned char *)subject, strlen(subject), tmp, uni_utf8_to_decomposed_titlecase);
    subject = str_c(tmp);
  }
  msg->subject_utf8_tcase = p_strdup(mailbox_source_get_messages_pool(source), subject);

  if (env->date == NULL) {
    msg->sent_date = (time_t)-1;
  } else if (!message_date_parse((const unsigned char *)env->date, strlen(env->date), &msg->sent_date,
                                 &msg->sent_date_tz)) {
    msg->sent_date = (time_t)-1;
  }
...
bool mailbox_global_get_sent_date(struct mailbox_source *source, struct message_global *msg, time_t *date_r,
                                  int *tz_r) {
  if (msg->sent_date == 0 && !mailbox_global_parse_envelope(source, msg))
    return FALSE;
...
bool mailbox_global_get_subject_utf8(struct mailbox_source *source, struct message_global *msg,
                                     const char **subject_r) {
  if (msg->subject_utf8_tcase == NULL && !mailbox_global_parse_envelope(source, msg))
    return FALSE;
...
void mailboxes_init(void) { hash_table_create(&storages, default_pool, 0, str_hash, strcmp); }

void mailboxes_deinit(void) { hash_table_destroy(&storages); }
