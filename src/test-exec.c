...
static void init_callback(struct imap_client *client, struct command *command, const struct imap_arg *args,
...
static unsigned int test_imap_match_args(struct test_exec_context *ctx, const struct imap_arg *match,
                                         const struct imap_arg *args, unsigned int max, bool prefix);
...
static void ATTR_FORMAT(2, 3) test_fail(struct test_exec_context *ctx, const char *fmt, ...) {
  ...
  if (!ctx->init_finished) {
    ...
    str = t_str_new(256);
    str_printfa(str,
                "*** Test %s command %u/%u (line %u)\n - failed: %s\n"
                " - Command",
                ctx->test->name, ctx->cur_group_idx + 1, array_count(&ctx->test->cmd_groups), cmd->linenum,
                t_strdup_vprintf(fmt, args));
    if (cmd->cur_cmd_tag != 0 && client != NULL) {
      str_printfa(str, " (tag %u.%u)", client->client.global_id, cmd->cur_cmd_tag);
    }
...
static struct test_command *test_cmd_find_by_cur_tag(struct test_command_group *group, unsigned int cur_cmd_tag) {
...
static const char *test_expand_relative_seq(struct test_exec_context *ctx, uint32_t seq) {
  const uint32_t *seqs;
  unsigned int count;
...
static void test_expand_all(struct test_exec_context *ctx, const char **_line, unsigned int *_line_len,
  ...
  for (line = p; line_len > 0;) {
    ...
    if (*line == '$') {
      ...
      if (str_to_uint32(var_name, &seq) == 0) {
        ...
        var_value = hash_table_lookup(ctx->variables, var_name);
        if (var_value == NULL && !skip_uninitialized) {
          test_fail(ctx, "Uninitialized variable: %s", var_name);
          break;
...
static const char *test_expand_input(struct test_exec_context *ctx, const char *str, const char *input) {
  ...
  for (; *str != '\0';) {
    ...
    if (str_to_uint32(var_name, &seq) == 0) {
      ...
      value = var_name[0] == '\0' ? NULL : hash_table_lookup(ctx->variables, var_name);
      if (value == NULL) {
        /* find how far we want to expand.
           FIXME: for now we just check the first
           letter */
        tmp_str = *str != '$' ? str : test_expand_input(ctx, str, input);
        p = input;
        while (i_toupper(*p) != i_toupper(*tmp_str) && *p != '\0')
          p++;
...
static unsigned int test_imap_match_list(struct test_exec_context *ctx, const struct imap_arg *match,
  ...
  for (; match->type != IMAP_ARG_EOL; match += chain_count, ret++) {
    for (i = 0; i < arg_count; i += chain_count) {
      if (test_imap_match_args(ctx, match, &args[i], chain_count, FALSE) == UINT_MAX) {
        matches[i] = 1;
        break;
      }
    }
    ...
    switch (match->type) {
      ...
      case IMAP_ARG_LITERAL:
        if (args->type == IMAP_ARG_LITERAL_SIZE) {
          /* shouldn't get here */
          i_panic(
              "Test failed because args "
              "contain literal size");
        }

        if (strcmp(imap_arg_as_astring(match), "$") == 0 && imap_arg_get_list(args, &listargs)) {
          /* "$" skips over a list */
          break;
        }
        if (!imap_arg_get_astring(args, &astr))
          return ret;
        mstr = test_expand_input(ctx, imap_arg_as_astring(match), astr);
        if (mstr == NULL)
          return ret;
      ...
      case IMAP_ARG_LIST:
        if (!imap_arg_get_list(args, &listargs))
          return ret;
        subret = test_imap_match_list(ctx, imap_arg_as_list(match), listargs);
        if (subret != UINT_MAX)
          return ret + subret;
        break;
...
static void test_handle_untagged_match(struct imap_client *client, const struct imap_arg *args) {
  ...
  if (!array_is_created(&(*groupp)->untagged)) {
    if (ctx->test->ignore_extra_untagged || client->client.state == STATE_LOGOUT) {
  ...
  if (imap_arg_get_atom(args, &str)) {
    if (strcasecmp(str, "ok") == 0 || strcasecmp(str, "no") == 0 || strcasecmp(str, "bad") == 0 ||
  ...
  cmd = array_idx(&(*groupp)->commands, 0);
  tag = cmd->cur_cmd_tag == 0 ? NULL : t_strdup_printf("%u.%u", client->client.global_id, cmd->cur_cmd_tag);
  ...
  for (i = 0; i < count; i++) {
    ...
    match_count = test_imap_match_args(ctx, untagged[i].args, args, UINT_MAX, prefix);
    if (tag != NULL)
    ...
    if (match_count == UINT_MAX) {
      if (untagged[i].existence == TEST_EXISTENCE_MUST_NOT_EXIST) {
        test_fail(ctx,
                  "Unexpected untagged match:\n"
                  "Match: %s\n"
                  "Reply: %s",
                  imap_args_to_str(untagged[i].args), imap_args_to_str(args));
        return;
      }
      found[i] = 1;
    ...
    if (maybes[i].count < match_count) {
      ...
      maybes[i].str = p_strdup(ctx->pool, imap_args_to_str(args));
    }
  ...
  if (!found_some) {
    ...
    if (!ctx->test->ignore_extra_untagged && ctx->first_extra_reply == NULL)
      ctx->first_extra_reply = p_strdup(ctx->pool, imap_args_to_str(args));
  }
}

static int test_handle_untagged(struct imap_client *client, const struct imap_arg *args) {
  struct test_exec_context *ctx = client->test_exec_ctx;
  ...
  if (imap_arg_get_atom(&args[0], &str) && imap_arg_atom_equals(&args[1], "EXPUNGE")) {
  ...
  if (imap_arg_get_atom(&args[0], &reply) && args[1].type == IMAP_ARG_LIST && IMAP_ARG_IS_NSTRING(&args[2]) &&
...
static void test_group_check_missing_untagged(struct test_exec_context *ctx, struct test_command_group *group) {
  ...
  for (i = 0; i < ut_count; i++) {
    if (ut[i].existence == TEST_EXISTENCE_MUST_EXIST && (i >= ctx->cur_received_untagged->used || found[i] == 0)) {
  ...
  if (missing_count != 0) {
    ...
    best_match = mcount < first_missing_idx ? NULL : maybes[first_missing_idx].str;
    ...
    test_fail(ctx,
              "Missing %u untagged replies "
              "(%u mismatches)\n"
              " - first unexpanded: %s\n"
              " - first expanded: %s\n"
              " - best match: %s",
              missing_count, ctx->cur_untagged_mismatch_count, imap_args_to_str(ut->args), str,
              best_match == NULL ? "" : best_match);
  }
}

static void test_group_output(struct test_exec_context *ctx, struct test_command_group *group) {
...
static void test_group_finished(struct test_exec_context *ctx, struct test_command_group *group) {
  if (array_is_created(&group->untagged))
    test_group_check_missing_untagged(ctx, group);

  if (ctx->first_extra_reply != NULL) {
    test_fail(ctx, "%u unexpected untagged replies received, first: %s", ctx->cur_untagged_mismatch_count,
...
static void test_cmd_callback(struct imap_client *client, struct command *command, const struct imap_arg *args,
                              enum command_reply reply) {
  struct test_exec_context *ctx = client->test_exec_ctx;
  struct test_command_group *const *groupp;
  ...
  hash_table_insert(ctx->variables, tag_hash_key, tag);
  match_count = test_imap_match_args(ctx, test_cmd->reply, args, UINT_MAX, TRUE);
  hash_table_remove(ctx->variables, tag_hash_key);

  if (match_count != UINT_MAX) {
    test_fail(ctx, "Expected tagged reply '%s', got '%s'", imap_args_to_str(test_cmd->reply), imap_args_to_str(args));
  }
...
static bool append_has_body(struct test_exec_context *ctx, const char *str_args, unsigned int str_args_len) {
  ARRAY_TYPE(imap_arg_list) * arg_list;
  const struct imap_arg *args;
  const char *error;

  /* mailbox [(flags)] ["datetime"] */
  arg_list = test_parse_imap_args(ctx->pool, str_args, str_args_len, &error);
  if (arg_list == NULL)
    return FALSE;
  ...
  return args[1].type == IMAP_ARG_LITERAL || args[1].type == IMAP_ARG_LITERAL_SIZE ||
         args[1].type == IMAP_ARG_LITERAL_SIZE_NONSYNC || imap_arg_atom_equals(&args[1], "catenate");
}

static void test_send_next_command(struct test_exec_context *ctx, struct imap_client *client,
                                   struct test_command_group *group, struct test_command *test_cmd) {
  ...
  if (strcasecmp(cmdline, "append") == 0) {
    ...
    (void)imap_client_append_full(client, NULL, NULL, NULL, test_cmd_callback, &cmd);
  } else if (strncasecmp(cmdline, "append ", 7) == 0 && !append_has_body(ctx, cmdline + 7, cmdline_len - 7)) {
    client->client.state = STATE_APPEND;
    (void)imap_client_append(client, cmdline + 7, FALSE, test_cmd_callback, &cmd);
  } else {
    if (test_cmd->linenum == 0 || strcasecmp(cmdline, "logout") == 0 || group->have_untagged_bye) {
    ...
    cmd = command_send_binary(client, cmdline, cmdline_len, test_cmd_callback, NULL);
...
static void test_send_next_command_group(struct test_exec_context *ctx) {
  ...
  array_foreach_modifiable(&(*groupp)->commands, cmd) test_send_next_command(ctx, client, *groupp, cmd);
...
static int test_send_no_commands(struct client *client ATTR_UNUSED) { return 0; }
...
static void test_send_first_command(struct test_exec_context *ctx) {
  ...
  for (i = 0; i < ctx->test->connection_count; i++)
    ctx->clients[i]->client.v.send_more_commands = test_send_no_commands;
...
static unsigned int mailbox_foreach(struct imap_client *client, ARRAY_TYPE(const_string) * mailboxes, const char *cmd) {
...
static void init_callback(struct imap_client *client, struct command *command, const struct imap_arg *args,
  ...
  if (reply == REPLY_BAD ||
      (reply == REPLY_NO && !(client->client.state == STATE_MDELETE && ctx->delete_refcount > 0))) {
    test_fail(ctx, "%s (tag %u.%u) failed: %s", command->cmdline, client->client.global_id, command->tag,
  ...
  if (ctx->listing) {
    ...
    ctx->listing = FALSE;
    if (array_count(&ctx->delete_mailboxes) > 0 || array_count(&ctx->unsubscribe_mailboxes) > 0) {
      ctx->delete_refcount += mailbox_foreach(client, &ctx->delete_mailboxes, "DELETE");
      ctx->delete_refcount += mailbox_foreach(client, &ctx->unsubscribe_mailboxes, "UNSUBSCRIBE");
      return;
    }
...
static void capability_callback(struct imap_client *client, struct command *command, const struct imap_arg *args,
                                enum command_reply reply) {
  struct test_exec_context *ctx = client->test_exec_ctx;
...
static void skip_logout_callback(struct imap_client *client ATTR_UNUSED, struct command *command ATTR_UNUSED,
                                 const struct imap_arg *args ATTR_UNUSED, enum command_reply reply ATTR_UNUSED) {}
...
static void test_skip(struct test_exec_context *ctx) {
  ...
  for (unsigned int i = 0; i < ctx->test->connection_count; i++) {
    ...
    command_send(ctx->clients[i], "LOGOUT", skip_logout_callback);
  }
}
...
static int test_send_lstate_commands(struct client *_client) {
  ...
  if (ctx->startup_state == ctx->test->startup_state &&
      (client->client.login_state != LSTATE_NONAUTH || ctx->test->startup_state == TEST_STARTUP_STATE_NONAUTH)) {
  ...
  switch (client->client.login_state) {
    ...
    case LSTATE_AUTH:
      ...
      if (client != ctx->clients[0] && ctx->startup_state != TEST_STARTUP_STATE_APPENDED) {
      ...
      switch (ctx->startup_state) {
        ...
        case TEST_STARTUP_STATE_DELETED:
          ...
          if (strcasecmp(client->storage->name, "INBOX") != 0) {
            str = t_strdup_printf("CREATE %s", t_imap_quote_str(client->storage->name));
            command_send(client, str, init_callback);
            break;
          }
        /* fall through */
        case TEST_STARTUP_STATE_CREATED:
          if (ctx->appends_left > 0 && (!mailbox_source_eof(ctx->source) || ctx->test->message_count != UINT_MAX)) {
            client->client.state = STATE_APPEND;
            ctx->appends_left--;
            if (imap_client_append_full(client, NULL, NULL, NULL, init_callback, &cmd) < 0)
              return -1;
            break;
          }
          /* finished appending */
        ...
        case TEST_STARTUP_STATE_APPENDED:
          str = t_strdup_printf("SELECT %s", t_imap_quote_str(client->storage->name));
          client->client.state = STATE_SELECT;
          command_send(client, str, init_callback);
          break;
...
static bool test_exec_client_disconnected(struct client *_client) {
  ...
  if ((*groupp)->have_untagged_bye && client->seen_bye) {
    ...
    array_foreach(&(*groupp)->commands, cmd) {
      if (!imap_arg_atom_equals(&cmd->reply[0], "") || cmd->reply[1].type != IMAP_ARG_EOL) {
...
static int test_execute(const struct test *test, struct tests_execute_context *exec_ctx) {
  ...
  ctx->cur_received_untagged = buffer_create_dynamic(default_pool, 128);
  ...
  for (i = 0; i < test->connection_count; i++) {
    ...
    ctx->clients[i]->client.v.send_more_commands = test_send_lstate_commands;
    ...
    key = i == 0 ? "username" : p_strdup_printf(pool, "username%u", i + 1);
    value = p_strdup(pool, t_strcut(ctx->clients[i]->client.user->username, '@'));
    hash_table_insert(ctx->variables, key, value);

    key = i == 0 ? "domain" : p_strdup_printf(pool, "domain%u", i + 1);
...
void test_execute_cancel_by_client(struct imap_client *client) {
  ...
  if (ctx->disconnects_waiting == 0) {
    if (!client->seen_bye || !test_execute_is_cur_group_with_bye(ctx))
