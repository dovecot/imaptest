...
static const char *get_astring(const char *str) {
  ...
  if (imap_parser_finish_line(parser, 1, 0, &args) > 0 && imap_arg_get_astring(&args[0], &ret))
    ret = t_strdup(ret);
...
static bool ends_with_literal(const unsigned char *line, const unsigned char *p, unsigned long *num_r) {
...
static void command_get_cmdline(struct imap_client *client, const char **_cmdline, unsigned int *_cmdline_len) {
...
struct command *command_send_binary(struct imap_client *client, const char *cmdline, unsigned int cmdline_len,
  ...
  if (argp == NULL) {
    /* error probably */
  } else if (strcasecmp(cmdname, "SELECT") == 0 || strcasecmp(cmdname, "EXAMINE") == 0) {
    ...
    if (name != NULL && strcmp(name, client->storage->name) != 0) {
      ...
      client->storage = mailbox_storage_get(source, client->client.user->username, name);
      client->view = mailbox_view_new(client->storage);
    }
  } else if (strcasecmp(cmdname, "DELETE") == 0 || strcasecmp(cmdname, "RENAME") == 0) {
    ...
    name = get_astring(argp);
    if (name != NULL) {
      storage = mailbox_storage_lookup(client->storage->source, client->client.user->username, name);
      if (storage != NULL)
