...
static void client_profile_init_mailbox(struct imap_client *client) {
  ...
  cache =
      user_get_mailbox_cache(client->client.user_client, client->storage->name);
  if (cache->uidvalidity != 0) return;
...
static void client_profile_send_missing_creates(struct imap_client *client) {
  ...
  if (client->client.user->profile->mail_inbox_reply_percentage > 0 ||
    ...
    if (imap_client_mailboxes_list_find(client, PROFILE_MAILBOX_DRAFTS) == NULL)
      command_send(client, "CREATE \"" PROFILE_MAILBOX_DRAFTS "\"",
                   state_callback);
    if (imap_client_mailboxes_list_find(client, PROFILE_MAILBOX_SENT) == NULL)
      command_send(client, "CREATE \"" PROFILE_MAILBOX_SENT "\"",
                   state_callback);
  }
}
...
int imap_client_profile_send_more_commands(struct client *_client) {
  ...
  if (array_count(&client->commands) > 0) return 0;
...
static void send_get_metadata_extension(struct imap_client *client, int uid) {
  ...
  if (client->client.user_client->profile->imap_metadata_extension != NULL &&
      array_is_created(&ms->fetch_m)) {
    imap_metadata_extension_query = replace_variable(
        client->client.user_client->profile->imap_metadata_extension,
        &ms->fetch_m);
    if (imap_metadata_extension_query == NULL) {
      cmd_str = t_strdup_printf(
          "GETMETADATA INBOX (%s)",
          client->client.user_client->profile->imap_metadata_extension);
...
static void client_profile_handle_fetch(struct imap_client *client,
  ...
  if (!imap_arg_get_list(list_arg, &args)) return;
  while (!IMAP_ARG_IS_EOL(args)) {
    if (!imap_arg_get_atom(args, &name)) return;
    args++;
    if (IMAP_ARG_IS_EOL(args)) return;
...
int imap_client_profile_handle_untagged(struct imap_client *client,
                                        const struct imap_arg *args) {
  if (imap_client_handle_untagged(client, args) < 0) return -1;

  if (client->client.login_state != LSTATE_SELECTED) return 0;
...
static struct imap_client *user_find_any_imap_client(struct user_client *uc) {
  ...
  array_foreach(&uc->clients, clientp) {
    ...
    if (last_client != NULL && last_client->client.idling) return last_client;
  }
  i_assert(last_client != NULL);
  return last_client;
}

static unsigned int user_get_timeout_interval(struct user *user,
...
static time_t user_get_next_timeout(struct user *user, time_t start_time,
                                    enum user_timestamp ts) {
  unsigned int interval = user_get_timeout_interval(user, ts);

  if (interval == 0) return (time_t)-1;
  return start_time + weighted_rand(interval);
}

static void user_mailbox_action_delete(struct imap_client *client,
                                       uint32_t uid) {
  const char *cmd;
...
static bool user_write_mail(struct user_client *uc) {
  ...
  if (uc->draft_uid == 0) {
    ...
    if (client2 != NULL && client2->client.state == STATE_IDLE) {
      imap_client_append_full(client2, PROFILE_MAILBOX_SENT, NULL, "",
                              state_callback, &cmd);
    }
...
static void user_mailbox_action_reply(struct imap_client *client,
                                      uint32_t uid) {
  const char *cmd;
...
static bool user_mailbox_action(struct user *user,
                                struct user_mailbox_cache *cache) {
  ...
  if (uid >= cache->uidnext) return FALSE;

  if (client->client.login_state != LSTATE_SELECTED) {
    i_warning("HEY client login state is != LSTATE_SELECTED its %d",
  ...
  if (!cache->last_action_uid_body_fetched) {
    ...
    cmd =
        t_strdup_printf("UID FETCH %u (%s)", uid,
  ...
  if (strcasecmp(cache->mailbox_name, "INBOX") != 0) return TRUE;
  ...
  if ((unsigned)i_rand() % 100 < user->profile->mail_inbox_delete_percentage) {
    user_mailbox_action_delete(client, uid);
  } else if ((unsigned)i_rand() % 100 <
             user->profile->mail_inbox_move_percentage) {
    user_mailbox_action_move(client, PROFILE_MAILBOX_SPAM, uid);
  } else if ((unsigned)i_rand() % 100 <
             user->profile->mail_inbox_reply_percentage) {
    user_mailbox_action_reply(client, uid);
  } else if ((unsigned)i_rand() % 100 <
                 user->profile->mail_inbox_search_percentage &&
             cache->last_action_uid_body_fetched) {
    user_mailbox_action_search(client);
  }
  cache->last_action_xuid_fetched = FALSE;
  return TRUE;
}
...
static bool user_client_is_connected(struct user_client *uc) {
  ...
  if (uc == NULL || array_count(&uc->clients) == 0) return FALSE;
    ...
    if ((*mailboxp)->next_action_timestamp <= ioloop_time &&
      ...
      (*mailboxp)->next_action_timestamp =
          user_mailbox_action(user, *mailboxp)
              ? (ioloop_time +
                 weighted_rand(user->profile->mail_action_repeat_delay))
              : (time_t)-1;
    }
...
static int user_timestamp_handle(struct user *user, enum user_timestamp ts,
                                 bool user_connected) {
  if (user->timestamps[ts] > ioloop_time) return -1;
  if (user->timestamps[ts] == (time_t)-1) {
    if (ts == USER_TIMESTAMP_LOGIN) {
      ...
      /* have to have a logout timestamp when there are
         connected clients. */
      user->timestamps[ts] = user_get_next_timeout(user, ioloop_time, ts);
  ...
  switch (ts) {
    ...
    case USER_TIMESTAMP_WRITE_MAIL:
      if (!user_connected) return 0;
      if (user_write_mail(user->active_client)) return 1;
...
static void user_run_actions(struct user *user) {
  ...
  if (disconnect_clients) {
    if (user_connected) user_logout(user->active_client);
    return;
  }
...
static void user_fill_timestamps(struct user *user, time_t start_time) {
  ...
  for (ts = 0; ts < USER_TIMESTAMP_COUNT; ts++) {
    ...
    user->timestamps[ts] =
        interval == 0 ? (time_t)-1 : (time_t)(start_time + i_rand() % interval);
    user_set_min_timestamp(user, user->timestamps[ts]);
  }
...
static void users_timeout(void *context ATTR_UNUSED) {
  ...
  array_foreach(users, userp) {
    if (ioloop_time < (*userp)->next_min_timestamp && !disconnect_clients) {
...
static void users_timeout_update(void) {
  if (to_users != NULL) timeout_remove(&to_users);
...
static void user_add_client_profile(struct user *user,
...
static void user_init_client_profiles(struct user *user,
