...
static const struct ssl_iostream_settings ssl_set = {.ca_dir = "ssl_client_ca_dir"};
...
static struct client *client_new_full(unsigned int i, struct user *user, struct user_client *uc) {
  if (client_min_free_idx == i)
    client_min_free_idx++;

  if (uc == NULL || uc->profile == NULL || strcmp(uc->profile->protocol, "imap") == 0)
...
int client_init(struct client *client, unsigned int idx, struct user *user, struct user_client *uc) {
  ...
  i_assert(idx >= array_count(&clients) || *(struct client **)array_idx(&clients, idx) == NULL);
  /*if (stalled) {
...
bool client_unref(struct client *client, bool reconnect) {
  ...
  if (disconnect_clients && !imaptest_has_clients())
  ...
  else if (io_loop_is_running(current_ioloop) && !no_new_clients && !disconnect_clients && reconnect) {
    if (client->logout_sent) {
      /* user successfully logged out, get another
         random user */
      if (client->user_client == NULL || client->user_client->profile == NULL)
    ...
    if (!stalled && (client->user_client == NULL || client->user_client->profile == NULL))
...
void clients_init(void) { i_array_init(&stalled_clients, CLIENTS_COUNT); }
