...
int imap_client_input_error(struct imap_client *client, const char *fmt, ...) {
  ...
  va_start(va, fmt);
  i_error("%s[%u]: %s: %s", client->client.user->username, client->client.global_id, t_strdup_vprintf(fmt, va),
          client->cur_args == NULL ? "" : imap_args_to_str(client->cur_args));
  va_end(va);
...
void imap_client_exists(struct imap_client *client, unsigned int msgs) {
  ...
  if (msgs < old_count) {
    imap_client_input_error(client, "Message count dropped %u -> %u", old_count, msgs);
...
static int imap_client_expunge(struct imap_client *client, unsigned int seq) {
  ...
  if (seq > count) {
    imap_client_input_error(client, "Tried to expunge sequence %u with only %u msgs", seq, count);
    return -1;
  }

  metadata = array_idx_get_space(&client->view->messages, seq - 1);
  if (metadata->fetch_refcount > 0) {
    imap_client_input_error(client, "Referenced message expunged seq=%u uid=%u", seq,
                            metadata->ms == NULL ? 0 : metadata->ms->uid);
    return -1;
  }
...
static void imap_client_expunge_uid_range(struct imap_client *client, const ARRAY_TYPE(seq_range) * expunged_uids) {
  const uint32_t *uidmap;
...
static void imap_client_enabled(struct imap_client *client, const struct imap_arg *args) {
  const char *str;
...
static int client_vanished(struct imap_client *client, const struct imap_arg *args) {
  ...
  if (!client->qresync_enabled) {
    imap_client_input_error(client, "Server sent VANISHED but we hadn't enabled QRESYNC");
    return -1;
  }

  if (imap_arg_get_list(args, &subargs)) {
    if (imap_arg_atom_equals(&subargs[0], "EARLIER") && IMAP_ARG_IS_EOL(&subargs[1])) {
  ...
  if (!imap_arg_get_atom(&args[0], &uidset) || !IMAP_ARG_IS_EOL(&args[1])) {
...
static void imap_client_list_result(struct imap_client *client, const struct imap_arg *args) {
  struct mailbox_list_entry *list;
  const char *name;

  if (args[0].type != IMAP_ARG_LIST || !IMAP_ARG_IS_NSTRING(&args[1]) || !imap_arg_get_astring(&args[2], &name))
    return;
...
void imap_client_mailboxes_list_begin(struct imap_client *client) {
  ...
  if (!array_is_created(&client->mailboxes_list))
    return;
  array_foreach_modifiable(&client->mailboxes_list, list) list->found = FALSE;
}
...
void imap_client_log_mailbox_view(struct imap_client *client) {
  ...
  for (i = 0; i < count; i++) {
    str_truncate(str, 0);
    str_printfa(str, "seq=%u uid=%u modseq=%llu flags=(", i + 1, uidmap[i], (unsigned long long)metadata[i].modseq);
    imap_write_flags(str, metadata[i].mail_flags, NULL);
    str_append(str, ") keywords=(");
    mailbox_view_keywords_write(client->view, metadata[i].keyword_bitmask, str);
    str_append(str, ")\r\n");
    write_full(client->client.rawlog_fd, str_data(str), str_len(str));
  }
...
struct mailbox_list_entry *imap_client_mailboxes_list_find(struct imap_client *client, const char *name) {
  struct mailbox_list_entry *list;
...
int imap_client_handle_untagged(struct imap_client *client, const struct imap_arg *args) {
  ...
  if (str_to_uint(str, &num) == 0) {
    ...
    if (num > array_count(&view->uidmap) && client->last_cmd->state > STATE_SELECT) {
      imap_client_input_warn(client, "seq too high (%u > %u, state=%s)", num, array_count(&view->uidmap),
      ...
      if (view->recent_count == array_count(&view->uidmap))
    ...
    view->last_thread_reply = IMAP_ARG_IS_EOL(args) ? i_strdup("") : i_strdup(imap_args_to_str(args + 1));
...
static int imap_client_input_args(struct imap_client *client, const struct imap_arg *args) {
  ...
  if (strcmp(tag, "+") == 0) {
    if (client->last_cmd == NULL) {
      return imap_client_input_error(client, "Unexpected command continuation");
    }
    client->last_cmd->callback(client, client->last_cmd, args, REPLY_CONT);
    return 0;
  }
  if (strcmp(tag, "*") == 0) {
  ...
  p = strchr(tag, '.');
  cmd = p != NULL && atoi(t_strdup_until(tag, p)) == (int)client->client.global_id
            ? command_lookup(client, atoi(t_strcut(p + 1, ' ')))
            : NULL;
  if (cmd == NULL) {
    return imap_client_input_error(client, "Unexpected tagged reply: %s", tag);
  }
...
static void imap_client_input(struct client *_client) {
  ...
  if (!client->seen_banner) {
    ...
    if (strncasecmp(line, "* PREAUTH ", 10) == 0) {
      ...
      imap_client_input_error(client, "Malformed banner \"%s\"", line);
    }
  ...
  while (imap_client_skip_literal(client)) {
    ret = imap_parser_read_args(client->parser, 0,
                                IMAP_PARSE_FLAG_LITERAL_SIZE | IMAP_PARSE_FLAG_LITERAL8 | IMAP_PARSE_FLAG_ATOM_ALLCHARS,
                                &imap_args);
    if (ret == -2) {
      /* need more data */
      break;
    ...
    if (ret < 0) {
      /* some error */
      imap_client_input_error(client, "error parsing input: %s", imap_parser_get_error(client->parser, &fatal));
      return;
    }
    if (imap_args->type == IMAP_ARG_EOL) {
      /* FIXME: we get here, but we shouldn't.. */
    } else {
      if (imap_parser_get_literal_size(client->parser, &literal_size)) {
        if (literal_size <= MAX_INLINE_LITERAL_SIZE) {
          /* read the literal */
          imap_parser_read_last_literal(client->parser);
          continue;
        }
      ...
      T_BEGIN { ret = imap_client_input_args(client, imap_args); }
      T_END;
...
static void imap_client_free(struct client *_client) {
  ...
  if (client->capabilities_list != NULL)
    p_strsplit_free(default_pool, client->capabilities_list);
  if (array_is_created(&client->mailboxes_list)) {
    array_foreach_modifiable(&client->mailboxes_list, list) i_free(list->name);
...
static const struct client_vfuncs imap_client_vfuncs = {.input = imap_client_input,
                                                        .output = imap_client_output,
                                                        .connected = imap_client_connected,
                                                        .logout = imap_client_logout,
                                                        .free = imap_client_free};

struct imap_client *imap_client_new(unsigned int idx, struct user *user, struct user_client *uc) {
  ...
  if (strchr(conf.mailbox, '%') != NULL || client->client.user_client != NULL)
  ...
  client->storage = mailbox_storage_get(user->mailbox_source, user->username, mailbox);
  client->view = mailbox_view_new(client->storage);

  client->client.v = imap_client_vfuncs;
  client->handle_untagged = user->profile != NULL ? imap_client_profile_handle_untagged : imap_client_handle_untagged;
  client->client.v.send_more_commands =
      user->profile != NULL ? imap_client_profile_send_more_commands : imap_client_plan_send_more_commands;
  return client;
}
