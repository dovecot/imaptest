...
#define BODY_NIL_REPLY "\"text\" \"plain\" NIL NIL NIL \"7bit\" 0 0"

#define ENVELOPE_NIL_REPLY "NIL NIL NIL NIL NIL NIL NIL NIL NIL NIL"
#define INTERNALDATE_NIL_TIMESTAMP 0
#define RFC822_SIZE_NIL_REPLY "0"

static void client_fetch_envelope(struct imap_client *client, struct message_metadata_dynamic *metadata,
                                  const struct imap_arg *args, unsigned int list_count, uint32_t uid) {
  const char *str, *message_id, *orig_str;

  if (list_count < 9 || !imap_arg_get_nstring(&args[9], &str) || str == NULL)
    return;
  orig_str = str;

  message_id = message_id_get_next(&str);
  if (message_id == NULL || message_id_get_next(&str) != NULL) {
    imap_client_input_warn(client, "UID %u has invalid Message-Id: %s", uid, orig_str);
    message_id = orig_str;
  }
  ...
  if (metadata->ms->msg != NULL) {
    ...
    imap_client_input_warn(client, "UID %u changed Message-Id: %s -> %s", uid, metadata->ms->msg->message_id,
                           message_id);
    return;
  }
  metadata->ms->msg = mailbox_source_get_msg(client->storage->source, message_id);
}

static const struct imap_arg *fetch_list_get(const struct imap_arg *args, const char *name) {
  const char *str;

  while (!IMAP_ARG_IS_EOL(args) && !IMAP_ARG_IS_EOL(&args[1])) {
    if (imap_arg_get_atom(args, &str) && strcasecmp(name, str) == 0)
      return &args[1];
    args += 2;
  }
  return NULL;
...
static bool have_unexpected_changes(struct imap_client *client, const struct msg_old_flags *old,
  ...
  if (old->kw_alloc_size != client->view->keyword_bitmask_alloc_size)
    return TRUE;
  return memcmp(old->keyword_bitmask, metadata->keyword_bitmask, old->kw_alloc_size) != 0;
}

static void check_unexpected_flag_changes(struct imap_client *client, const struct msg_old_flags *old,
                                          const struct message_metadata_dynamic *metadata) {
  ...
  for (i = 0; i < N_ELEMENTS(storage->flags_owner_client_idx1); i++) {
    ...
    if (old_set != new_set && storage->flags_owner_client_idx1[i] == client->client.idx + 1) {
      imap_client_state_error(client, "Owned flag changed: %s%s", mail_flag_names[i], expunge_state);
    }
  }
  ...
  for (i = 0; i < new_alloc_size; i++) {
    old_set = i >= old->kw_alloc_size ? FALSE : (old->keyword_bitmask[i / 8] & (1 << (i % 8))) != 0;
    new_set = (metadata->keyword_bitmask[i / 8] & (1 << (i % 8))) != 0;
    if (old_set != new_set && keywords[i].name->owner_client_idx1 == client->client.idx + 1) {
      imap_client_state_error(client, "Owned keyword changed: %s%s", keywords[i].name->name, expunge_state);
    }
  }
}

static void message_metadata_set_flags(struct imap_client *client, const struct imap_arg *args,
  ...
  old_flags.keyword_bitmask = old_flags.kw_alloc_size == 0 ? NULL : t_malloc0(old_flags.kw_alloc_size);
  if (metadata->keyword_bitmask != NULL) {
    memcpy(old_flags.keyword_bitmask, metadata->keyword_bitmask, old_flags.kw_alloc_size);
  }

  mailbox_keywords_clear(view, metadata);
  while (!IMAP_ARG_IS_EOL(args)) {
    if (!imap_arg_get_atom(args, &atom)) {
      imap_client_input_error(client, "Flags list contains non-atoms.");
      return;
    }
    ...
    if (*atom == '\\') {
      ...
      if (flag != 0)
        ...
        imap_client_input_error(client, "Invalid system flag: %s", atom);
      }
    } else {
  ...
  if ((old_flags.flags & MAIL_FLAGS_SET) == 0) {
  ...
  } else if (metadata->flagchange_dirty_type != FLAGCHANGE_DIRTY_NO || client->qresync_select_cache != NULL) {
    ...
    if (have_unexpected_changes(client, &old_flags, metadata)) {
      imap_client_state_error(client, "Flags unexpectedly changed for owned message");
    }
  } else if (client->storage->assign_flag_owners)
    check_unexpected_flag_changes(client, &old_flags, metadata);

  if ((flags & MAIL_RECENT) != 0 && metadata->ms != NULL && !client->storage->dont_track_recent) {
    if (metadata->ms->recent_client_global_id == 0) {
      if (client->view->readwrite) {
        metadata->ms->recent_client_global_id = client->client.global_id;
      }
    } else if (metadata->ms->recent_client_global_id != client->client.global_id) {
      imap_client_state_error(client,
                              "Message UID=%u has \\Recent flag in "
                              "multiple sessions: %u and %u",
                              metadata->ms->uid, client->client.global_id, metadata->ms->recent_client_global_id);
  ...
  if (metadata->fetch_refcount <= 1) {
    ...
    if (metadata->flagchange_dirty_type != FLAGCHANGE_DIRTY_NO) {
      metadata->flagchange_dirty_type = FLAGCHANGE_DIRTY_MAYBE;
    }
  } else if (metadata->flagchange_dirty_type == FLAGCHANGE_DIRTY_YES)
    metadata->flagchange_dirty_type = FLAGCHANGE_DIRTY_WAITING;
}

static void message_metadata_set_modseq(struct imap_client *client, const char *value,
                                        struct message_metadata_dynamic *metadata) {
  uint64_t modseq;
  uint32_t uid = metadata->ms == NULL ? 0 : metadata->ms->uid;

  if (str_to_uint64(value, &modseq) < 0 || modseq == 0) {
    imap_client_input_error(client, "UID=%u Invalid MODSEQ %s returned", uid, value);
    return;
  }
  if (modseq < metadata->modseq) {
    imap_client_input_warn(client, "UID=%u MODSEQ dropped %s -> %s", uid, dec2str(metadata->modseq), dec2str(modseq));
  }

  if (metadata->flagchange_dirty_type != FLAGCHANGE_DIRTY_NO || client->qresync_select_cache != NULL) {
    /* we're changing the flags ourself */
  } else if (metadata->ms == NULL) {
    /* UID now known yet, don't do any owning checks */
  } else if (metadata->ms->owner_client_idx1 == client->client.idx + 1 && modseq != metadata->modseq) {
    imap_client_state_error(client, "UID=%u MODSEQ changed for owned message: %s -> %s", uid, dec2str(metadata->modseq),
                            dec2str(modseq));
  }
...
static void headers_parse(struct imap_client *client, struct istream *input, ARRAY_TYPE(message_header) * headers_arr) {
  ...
  while ((ret = message_parse_header_next(parser, &hdr)) > 0) {
    ...
    if (i == count) {
      imap_client_state_error(client, "Unexpected header in reply: %s", hdr->name);
    } else if (headers[i].value_len == 0) {
      /* first header */
      new_value = hdr->full_value_len == 0 ? NULL : t_malloc_no0(hdr->full_value_len);
      ...
      new_value = t_malloc_no0(headers[i].value_len + 1 + hdr->full_value_len);
      memcpy(new_value, headers[i].value, headers[i].value_len);
      new_value[headers[i].value_len] = '\n';
      memcpy(new_value + 1 + headers[i].value_len, hdr->full_value, hdr->full_value_len);
      headers[i].value = new_value;
      headers[i].value_len += hdr->full_value_len;
    }
  }
...
static void headers_match(struct imap_client *client, ARRAY_TYPE(message_header) * headers_arr,
  ...
  for (i = 0; i < fetch_count; i++) {
    for (j = 0; j < orig_count; j++) {
      if (strcasecmp(fetch_headers[i].name, orig_headers[j].name) == 0)
        break;
    }
    if (j == orig_count) {
      ...
      if (msg_header.value_len != 0) {
        ...
        memcpy(value, fetch_headers[i].value, msg_header.value_len);
      ...
      orig_headers = array_get(&msg->headers, &orig_count);
               ...
               memcmp(fetch_headers[i].value, orig_headers[j].value, fetch_headers[i].value_len) != 0) {
      imap_client_state_error(client, "%s: Header %s changed '%.*s' -> '%.*s'", msg->message_id, fetch_headers[i].name,
                              (int)orig_headers[j].value_len, (const char *)orig_headers[j].value,
                              (int)fetch_headers[i].value_len, (const char *)fetch_headers[i].value);
    }
  }
}
static int fetch_parse_header_fields(struct imap_client *client, const struct imap_arg *args,
  ...
  for (arg = header_args; !IMAP_ARG_IS_EOL(arg); arg++) {
    ...
    /* drop duplicates */
    for (i = 0; i < fetch_count; i++) {
      if (strcasecmp(fetch_headers[i].name, msg_header.name) == 0)
        break;
    }
...
static void fetch_parse_body1(struct imap_client *client, const struct imap_arg *arg,
  ...
  if (len > 0) {
    ...
    /* make sure there are no non-ascii characters, since we don't
       ...
       chars. */
    for (i = 0; i < len; i++) {
      if (body[start + i] < 32 || (unsigned char)body[start + i] >= 0x80) {
        len = i;
        break;
      }
    }
...
void mailbox_state_handle_fetch(struct imap_client *client, unsigned int seq, const struct imap_arg *args) {
  struct mailbox_view *view = client->view;
  struct message_metadata_dynamic *metadata;
  ...
  if (arg == NULL && client->qresync_enabled) {
    imap_client_input_error(client, "FETCH didn't return UID while QRESYNC was enabled");
  }
  if (arg != NULL && imap_arg_get_atom(arg, &value)) {
    if (str_to_uint32(value, &uid) < 0 || uid == 0)
      imap_client_input_error(client, "Invalid UID number %s", value);
    else if (*uidp == 0) {
      view->known_uid_count++;
      *uidp = uid;
    } else if (*uidp != uid) {
      imap_client_input_error(client, "UID changed for sequence %u: %u -> %u", seq, *uidp, uid);
      uid_changed = TRUE;
      *uidp = uid;
    }
  }
  ...
  if (metadata->ms == NULL && uid != 0) {
    metadata->ms = message_metadata_static_get(client->storage, uid);
  ...
  if (metadata->ms != NULL) {
    ...
    if (arg != NULL) {
      ...
      if (!imap_arg_get_list_full(arg, &env_args, &env_list_count)) {
        imap_client_input_error(client, "FETCH ENVELOPE didn't return a list");
        return;
      }

      client_fetch_envelope(client, metadata, env_args, env_list_count, uid);
    }
  }
  ...
  for (i = 0; i + 1 < list_count; i += 2) {
    ...
    if (strcmp(name, "FLAGS") == 0) {
      if (listargs == NULL) {
        imap_client_input_error(client, "FLAGS reply isn't a list");
        continue;
      }
    ...
    if (strcmp(name, "INTERNALDATE") == 0) {
      ...
      if (!imap_parse_datetime(value, &t, &tz_offset)) {
        imap_client_input_error(client, "Broken INTERNALDATE");
      ...
      } else if (metadata->ms->internaldate != t || metadata->ms->internaldate_tz != tz_offset) {
        imap_client_input_error(client,
                                "UID=%u INTERNALDATE changed "
                                "%s+%d -> %s+%d",
                                uid, dec2str(metadata->ms->internaldate), metadata->ms->internaldate_tz, dec2str(t),
                                tz_offset);
      }
      continue;
    }
    ...
    if (strcmp(name, "BODY") == 0) {
      if (strncasecmp(value, BODY_NIL_REPLY, strlen(BODY_NIL_REPLY)) == 0)
        continue;
      ...
      if (strncasecmp(value, ENVELOPE_NIL_REPLY, strlen(ENVELOPE_NIL_REPLY)) == 0)
        continue;
      ...
      if (strcmp(name + 5, "HEADER.FIELDS") == 0) {
        if (fetch_parse_header_fields(client, args + i + 1, metadata->ms) < 0) {
          imap_client_input_error(client, "Broken HEADER.FIELDS");
        }
        ...
        fetch_parse_body1(client, &args[i + 1], metadata->ms);
    ...
    if (p != NULL && (*p == NULL || strcasecmp(*p, value) != 0)) {
      if (*p != NULL) {
        imap_client_state_error(client, "uid=%u %s: %s changed '%s' -> '%s'", metadata->ms->uid,
                                metadata->ms->msg->message_id, name, *p, value);
      }
      *p = p_strdup(mailbox_source_get_messages_pool(view->storage->source), value);
    } else if (sizep != NULL) {
      ...
      if (*sizep != value_size && *sizep != 0) {
        imap_client_state_error(client, "uid=%u %s: %s size changed %" PRIuUOFF_T " -> '%" PRIuUOFF_T "'",
                                metadata->ms->uid, metadata->ms->msg->message_id, name, *sizep, value_size);
      }
      *sizep = value_size;
    }
  }
  if (i != list_count)
    imap_client_input_error(client, "FETCH reply doesn't have key-value pairs (i=%d,list_count=%d)", i, list_count);

  /* assign owner only after processing FETCH so that we don't think the
     FETCH changes caused a change yet */
  if (metadata->ms != NULL) {
    message_metadata_static_assign_owner(client->storage, metadata->ms);
  }
}

int mailbox_state_set_flags(struct mailbox_view *view, const struct imap_arg *args) {
  const struct mailbox_keyword *keywords;
  struct mailbox_keyword *kw;
  ...
  for (i = 0; i < count; i++) {
    if (keywords[i].flags_counter != view->flags_counter && keywords[i].msg_refcount > 0) {
      i_error(
          "Keyword '%s' dropped, but it still had "
          "%u references",
          keywords[i].name->name, keywords[i].msg_refcount);
      errors = TRUE;
    }
  }
