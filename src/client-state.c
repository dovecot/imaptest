...
bool do_rand(enum client_state state) { return (i_rand() % 100) < states[state].probability; }

bool do_rand_again(enum client_state state) { return (i_rand() % 100) < states[state].probability_again; }
...
static enum client_state client_eat_first_plan(struct imap_client *client) {
  ...
  client->plan_size--;
  memmove(client->plan, client->plan + 1, sizeof(client->plan[0]) * client->plan_size);
  return state;
}
...
static enum client_state client_get_next_state(enum client_state state) {
  ...
  for (;;) {
    if (!conf.random_states)
      ...
      state = STATE_LIST + (i_rand() % (STATE_LOGOUT - STATE_LIST + 1));
    }
...
static enum state_flags client_get_pending_cmd_flags(struct imap_client *client, enum login_state *new_lstate_r) {
...
static enum client_state client_update_plan(struct imap_client *client) {
  ...
  state = client->plan_size > 0 ? client->plan[client->plan_size - 1] : client->plan[0];
  if (client->plan_size > 0 && (states[state].flags & FLAG_STATECHANGE) != 0) {
    /* wait until the state change is done before making new
       commands. */
    return client->plan[0];
  }
  if ((client_get_pending_cmd_flags(client, &lstate) & FLAG_STATECHANGE) != 0)
    return state;

  if (state == STATE_LOGOUT)
    return state;

  while (client->plan_size < sizeof(client->plan) / sizeof(client->plan[0])) {
    switch (client->client.login_state) {
      case LSTATE_NONAUTH:
        /* we begin with LOGIN/AUTHENTICATE commands */
        i_assert(client->plan_size == 0);
        state = do_rand(STATE_AUTHENTICATE) ? STATE_AUTHENTICATE : STATE_LOGIN;
        break;
      case LSTATE_AUTH:
      case LSTATE_SELECTED:
        if (!do_rand_again(state)) {
          ...
          } while (state == STATE_UIDFETCH && client->uid_fetch_performed);
        }
        break;
    }
    i_assert(state <= STATE_LOGOUT);
    ...
    if (states[state].login_state > client->client.login_state ||
        (client->client.login_state != LSTATE_NONAUTH && (state == STATE_AUTHENTICATE || state == STATE_LOGIN))) {
      /* can't do this now */
      continue;
    }
...
static bool client_pending_cmds_allow_statechange(struct imap_client *client, enum client_state state) {
  ...
  while (array_count(&client->commands) < MAX_COMMAND_QUEUE_LEN) {
    ...
    if ((states[state].flags & FLAG_STATECHANGE) != 0) {
      /* this command would change the state. check if there
         ...
         change */
      if (!client_pending_cmds_allow_statechange(client, state))
        break;
    }
    pending_flags = client_get_pending_cmd_flags(client, &new_lstate);
    if ((states[state].flags & FLAG_STATECHANGE) == 0 && (pending_flags & FLAG_STATECHANGE) != 0) {
      ...
      if (new_lstate <= states[state].login_state || _client->login_state < states[state].login_state)
        break;
    }
    if ((states[state].flags & FLAG_MSGSET) != 0 && (pending_flags & (FLAG_EXPUNGES | FLAG_STATECHANGE)) != 0) {
...
int imap_client_append_full(struct imap_client *client, const char *mailbox, const char *flags, const char *datetime,
  ...
  args = t_str_new(128);
  if (!client->append_unfinished) {
    str_printfa(args, "\"%s\"", mailbox != NULL ? mailbox : client->storage->name);
  }
  if (flags != NULL)
  ...
  return imap_client_append(client, str_c(args), add_datetime, callback, cmd_r);
}
...
int imap_client_append_random(struct imap_client *client) {
  ...
  if ((i_rand() % 2) == 0) {
    flags = mailbox_view_get_random_flags(client->view, client->client.idx);
  }
  if ((i_rand() % 2) == 0)
    datetime = "";
  return imap_client_append_full(client, NULL, flags, datetime, state_callback, &cmd);
}

static void metadata_update_dirty(struct message_metadata_dynamic *metadata, bool set) {
  if (set) {
...
static void seq_range_flags_ref(struct imap_client *client, const ARRAY_TYPE(seq_range) * seq_range, int diff,
                                bool update_dirty) {
...
static bool store_verify_parse(struct store_verify_context *ctx, struct imap_client *client, char type,
                               const char *flags) {
  ...
  max_size = I_MAX(client->view->keyword_bitmask_alloc_size, (array_count(&client->view->keywords) + 7) / 8);
  ...
  for (tmp = t_strsplit(flags, " "); *tmp != NULL; tmp++) {
    if (**tmp == '\\')
      ...
      imap_client_state_error(client, "STORE didn't create keyword: %s", *tmp);
    } else {
      /* @UNSAFE */
...
static bool store_verify_seq(struct store_verify_context *ctx, uint32_t seq) {
  struct message_metadata_dynamic *metadata;
  ...
  expunge_state = metadata->ms == NULL ? "?" : metadata->ms->expunged ? "yes" : "no";
  if ((metadata->mail_flags & MAIL_FLAGS_SET) == 0 || metadata->flagchange_dirty_type == FLAGCHANGE_DIRTY_YES) {
  ...
  if (test_flags != test_flags_result) {
    imap_client_state_error(ctx->client, "STORE didn't update flags for seq %u (expunged=%s)", seq, expunge_state);
    return FALSE;
  }
...
static void store_verify_result(struct imap_client *client, char type, const char *flags,
...
void imap_client_handle_resp_text_code(struct imap_client *client, const struct imap_arg *args) {
  ...
  if (strcmp(key, "READ-WRITE") == 0)
    ...
    if (str_to_uint64(value, &view->highest_modseq) < 0) {
      imap_client_input_warn(client, "Invalid HIGHESTMODSEQ %s", value);
    }
    ...
    if (client->client.login_state != LSTATE_SELECTED) {
      imap_client_input_warn(client, "CLOSED code sent in non-selected state %d", client->client.login_state);
    } else {
    ...
    if (new_uidvalidity != view->storage->uidvalidity) {
      if (view->storage->uidvalidity != 0 && !conf.no_tracking) {
        i_error("UIVALIDITY changed: %u -> %u", view->storage->uidvalidity, new_uidvalidity);
      }
      view->storage->uidvalidity = new_uidvalidity;
    }
    if (client->qresync_select_cache != NULL &&
        client->qresync_select_cache->uidvalidity == view->storage->uidvalidity) {
      /* remember how many messages we currently have */
      client->qresync_pending_exists = array_count(&view->uidmap);
      mailbox_view_restore_offline_cache(view, client->qresync_select_cache);
...
void imap_client_handle_tagged_reply(struct imap_client *client, struct command *cmd, const struct imap_arg *args,
  ...
  if (strncasecmp(cmd->cmdline, "select ", 7) == 0 || strncasecmp(cmd->cmdline, "examine ", 8) == 0) {
    if (reply == REPLY_OK)
  ...
  } else if (strcasecmp(cmd->cmdline, "close") == 0 || strcasecmp(cmd->cmdline, "unselect") == 0) {
    if (reply == REPLY_OK && client->client.login_state == LSTATE_SELECTED)
      imap_client_mailbox_close(client);
  }
...
static int client_handle_cmd_reply(struct imap_client *client, struct command *cmd, const struct imap_arg *args,
  ...
  switch (reply) {
    ...
    case REPLY_NO:
      ...
      switch (cmd->state) {
        ...
        case STATE_STORE_DEL:
          ...
          if (strcmp(line, "STORE completed") == 0 || strcmp(line, "STORE failed") == 0) {
            /* Zimbra */
            break;
          }
    ...
    case REPLY_CONT:
      ...
      imap_client_input_error(client, "%s: Unexpected continuation", states[cmd->state].name);
      return -1;
  }
  ...
  switch (cmd->state) {
    ...
    case STATE_LOGIN:
      ...
      for (i = 0; i < 3 && !stalled && !no_new_clients; i++) {
        ...
        client_new_random(array_count(&clients), client->client.user->mailbox_source);
      }
      break;
    case STATE_LIST:
      if (reply == REPLY_OK)
    ...
    case STATE_STORE_DEL: {
      ...
      if (!silent && client->storage->assign_flag_owners && reply == REPLY_OK) {
        i_assert(type != '\0');
    ...
    case STATE_COPY:
      if (reply == REPLY_NO) {
        if (imap_arg_atom_equals(args, "[TRYCREATE]")) {
          str = t_strdup_printf("CREATE \"%s\"", conf.copy_dest);
...
bool imap_client_get_random_seq_range(struct imap_client *client, ARRAY_TYPE(seq_range) * range, unsigned int count,
  ...
  dirty_flags = flag_type == CLIENT_RANDOM_FLAG_TYPE_STORE || flag_type == CLIENT_RANDOM_FLAG_TYPE_STORE_SILENT;
  msgs = array_count(&client->view->uidmap);
  for (i = tries = 0; i < count && tries < count * 3; tries++) {
    seq = i_rand() % msgs + 1;
    metadata = array_idx_get_space(&client->view->messages, seq - 1);
    owner = metadata->ms == NULL ? 0 : metadata->ms->owner_client_idx1;
...
static void client_dirty_all_flags(struct imap_client *client, ARRAY_TYPE(seq_range) * seq_range,
                                   enum client_random_flag_type flag_type) {
...
static void seq_range_to_imap_range(const ARRAY_TYPE(seq_range) * seq_range, string_t *dest) {
  const struct seq_range *range;
  unsigned int i, count;
...
static void client_select_qresync(struct imap_client *client) {
  ...
  if (cache != NULL) {
    ...
    str_printfa(cmd, " (QRESYNC (%u %llu))", cache->uidvalidity, (unsigned long long)cache->highest_modseq);
    cache->refcount++;
...
int imap_client_plan_send_next_cmd(struct imap_client *client) {
  ...
  switch (state) {
    ...
    case STATE_LOGIN:
      ...
      str = t_strdup_printf("LOGIN \"%s\" \"%s\"", str_escape(_client->user->username),
    ...
    case STATE_MCREATE:
      if (i_rand() % 2 != 0)
        str = t_strdup_printf("CREATE \"test%c%d\"", IMAP_HIERARCHY_SEP, i_rand() % 20);
      else
        str = t_strdup_printf("CREATE \"test%c%d%c%d\"", IMAP_HIERARCHY_SEP, i_rand() % 20, IMAP_HIERARCHY_SEP,
                              i_rand() % 20);
      command_send(client, str, state_callback);
      break;
    case STATE_MSUBS: {
      ...
      if (i_rand() % 2 != 0)
        str = t_strdup_printf("%s \"test%c%d\"", cmd, IMAP_HIERARCHY_SEP, i_rand() % 20);
      else
        str = t_strdup_printf("%s \"test%c%d%c%d\"", cmd, IMAP_HIERARCHY_SEP, i_rand() % 20, IMAP_HIERARCHY_SEP,
                              i_rand() % 20);
      command_send(client, str, state_callback);
      break;
    }
    case STATE_MDELETE:
      if (i_rand() % 2 != 0)
        str = t_strdup_printf("DELETE \"test%c%d\"", IMAP_HIERARCHY_SEP, i_rand() % 20);
      else
        str = t_strdup_printf("DELETE \"test%c%d%c%d\"", IMAP_HIERARCHY_SEP, i_rand() % 20, IMAP_HIERARCHY_SEP,
                              i_rand() % 20);
      command_send(client, str, state_callback);
      break;
    case STATE_SELECT:
    ...
    case STATE_UIDFETCH:
      ...
      break;
    case STATE_FETCH: {
      static const char *fields[] = {"UID",  "FLAGS",         "ENVELOPE",   "INTERNALDATE",
                                     "BODY", "BODYSTRUCTURE", "RFC822.SIZE"};
      static const char *header_fields[] = {"From",       "To",          "Cc",         "Subject",
                                            "References", "In-Reply-To", "Message-ID", "Delivered-To"};
      count = I_MIN(msgs, 100);
      if (!imap_client_get_random_seq_range(client, &seq_range, count, CLIENT_RANDOM_FLAG_TYPE_FETCH))
        break;
      ...
      for (i = (i_rand() % 4) + 1; i > 0; i--) {
        if ((i_rand() % 4) != 0) {
          str_append(cmd, fields[i_rand() % N_ELEMENTS(fields)]);
        } else {
          str_append(cmd, "BODY.PEEK[HEADER.FIELDS (");
          for (j = (i_rand() % 4) + 1; j > 0; j--) {
            int idx = i_rand() % N_ELEMENTS(header_fields);
    ...
    case STATE_FETCH2: {
      static const char *fields[] = {"BODY.PEEK[HEADER]", "RFC822.HEADER", "BODY.PEEK[]", "BODY.PEEK[1]",
                                     "BODY.PEEK[TEXT]"};
      /* Fetch also UID so that error logging can show it in
         case of problems */
      str = t_strdup_printf("FETCH %lu (UID %s)", (random() % msgs) + 1, fields[i_rand() % N_ELEMENTS(fields)]);
      command_send(client, str, state_callback);
      break;
    }
    case STATE_SEARCH:
      ...
      break;
    case STATE_SORT: {
      static const char *fields[] = {"ARRIVAL", "CC", "DATE", "FROM", "SIZE", "SUBJECT", "TO"};
      cmd = t_str_new(512);
      str_append(cmd, "SORT (");
      ...
      switch (i_rand() % 3) {
        ...
        case 1:
          ...
          break;
        case 2:
          str_printfa(cmd, "%u:%u", (i_rand() % msgs) + 1, (i_rand() % msgs) + 1);
          break;
      }
      command_send(client, str_c(cmd), state_callback);
      break;
    ...
    case STATE_COPY:
      ...
      str = t_strdup_printf("COPY %u:%u %s", seq1, seq2, conf.copy_dest);
      command_send(client, str, state_callback);
      break;
    case STATE_STORE:
      count = i_rand() % (msgs < 10 ? msgs : I_MIN(msgs / 5, 50));
      flag_type = conf.checkpoint_interval == 0 && i_rand() % 2 == 0 ? CLIENT_RANDOM_FLAG_TYPE_STORE_SILENT
                                                                     : CLIENT_RANDOM_FLAG_TYPE_STORE;
      if (!imap_client_get_random_seq_range(client, &seq_range, count, flag_type))
        break;
      ...
      str_printfa(cmd, " (%s)", mailbox_view_get_random_flags(client->view, _client->idx));

      icmd = command_send(client, str_c(cmd), state_callback);
      icmd->seq_range = seq_range;
      break;
    case STATE_STORE_DEL:
      owner = client->storage->flags_owner_client_idx1[MAIL_FLAG_DELETED_IDX];
      if (client->storage->assign_flag_owners && owner != _client->idx + 1) {
      ...
      count = 1 + (states[STATE_APPEND].probability + states[state].probability / 2) / (states[state].probability);
      ...
      if (!client->storage->seen_all_recent && !client->storage->assign_msg_owners && conf.checkpoint_interval != 0 &&
          msgs > 0) {
    ...
    case STATE_APPEND:
      ...
      break;
    case STATE_STATUS:
      str = t_strdup_printf("STATUS \"%s\" (MESSAGES UNSEEN RECENT)", client->storage->name);
      command_send(client, str, state_callback);
      break;
    case STATE_NOOP:
